...
def read_screen_names(filename):
    ...
    with open(filename) as f:
      content = f.readlines()
    f.close()
    return [name.strip() for name in content]
...
def get_users(twitter, screen_names):
  ...
  """
  ###TODO
  names = str(screen_names).replace("'","").strip('[]')
  rsp_obj = robust_request(twitter, 'users/lookup', {'screen_name':names})
  return [user for user in rsp_obj]
...
def get_friends(twitter, screen_name):
  ...
  ###TODO
  ids = []
  rsp_obj = robust_request(twitter,'friends/ids',{'screen_name':screen_name})
  for item in rsp_obj.get_iterator():
    ids.append(item)
  return sorted(ids)
...
def print_num_friends(users):
  ...
  """
  ###TODO
  temp = [(user['screen_name'],len(user['friends'])) for user in users]
  temp.sort(key=lambda item:item[0]) # sorted by candidate name
  for item in temp:
...
def friend_overlap(users):
  ...
  combinations = list(itertools.combinations(screen_names,2)) # returns AB, AC, AD ...
  
  tup = {}
  for name in combinations: 
    for user in users:
      if name[0] == user['screen_name'] or name[1] == user['screen_name']:
        if name not in tup:
          tup[name] = user['friends']
        else:
          tup[name] = set(user['friends']).intersection(tup[name])
  res = [(name[0], name[1], len(tup[name])) for name in combinations]
  return (sorted(tuple(res), key=lambda x:(-x[2],x[0],x[1])))
...
def followed_by_hillary_and_donald(users, twitter):
  ...
  intersection = set(followed_by_hillary).intersection(followed_by_trump) # returns ID
  if len(intersection) >= 1: # assuming that they might follow more in the future
    for item in intersection:
      r = robust_request(twitter, 'users/lookup', {'user_id': item})
      for result in r.get_iterator():
...
def create_graph(users, friend_counts):
  ...
  ###TODO
  G = nx.Graph()
  friends = [friend for friend in friend_counts if friend_counts[friend] > 1]
  for user in users:
    G.add_node(user['screen_name'])
    for friend in friends:
      G.add_node(friend)
      if friend in user['friends']:
        G.add_edge(friend, user['screen_name'])
  return G          
...
def draw_network(graph, users, filename):
  ...
  nx.draw_networkx_labels(graph, labels=labels, pos=pos,font_size=6)
