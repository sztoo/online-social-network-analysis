...
def bfs(graph, root, max_depth):
    ...
    while (len(deq) != 0):
      n = deq.popleft()
      n_children = graph.neighbors(n)
      if(distance[n] > max_depth):
        break
      for child in n_children:
        if(visited[child] == "no"):
          if(distance[child] >= distance[n] + 1):
...
def bottom_up(root, node2distances, node2num_paths, node2parents):
    ...
    ###TODO
    # pass
    max_dist = max(node2distances.values())
    node_max_dist = [k for k,v in node2distances.items() if v==max_dist]
    ...
    for edge in edges:
      if(root in edge): 
        root_idx = edge.index(root)
        if(root_idx > 0): 
          res[edge] = (node_weight[edge[0]]/len(node2parents[edge[0]]))
        else:
          res[edge] = (node_weight[edge[1]]/len(node2parents[edge[1]]))
...
def approximate_betweenness(graph, max_depth):
    ...
    for node in nodes:
      ...
      sf = bottom_up(node, node2distances, node2num_paths, node2parents)
      for edge in sf:
        res[edge] += sf[edge]
    for edge in res:
      res[edge] = (res[edge]/(2.0))
...
def partition_girvan_newman(graph, max_depth):
    ...
    ###TODO
    # pass
    approx_btw = approximate_betweenness(graph, max_depth)
    components = [c for c in nx.connected_component_subgraphs(graph)]
    graph_copy = graph.copy()
    sorted_dict = sorted(approx_btw.items(), key=lambda x:(-x[1],x[0][0], x[0][1]))
...
def norm_cut(S, T, graph):
    ...
    ###TODO
    # pass
    norm_cut_val = (cut(S,T,graph)/volume(S,graph)) + (cut(S,T,graph)/volume(T,graph))
    return norm_cut_val
...
def score_max_depths(graph, max_depths):
    ...
    for max_d in max_depths:
      c = partition_girvan_newman(graph, max_d)
      res.append((max_d, norm_cut(c[0], c[1], graph)))
    return res
...
# - $n_{x,y,i}$ is the number of shortest paths between $x$ and $y$ with length $i$
    ...
    nn = set(graph.neighbors(node)) # nn - neighbor nodes
    graph_nodes = graph.nodes()
    e_score_list = []
    for n in graph_nodes:
      if(n not in graph.neighbors(node)):
        nn2 = set(graph.neighbors(n))
        jacc = (len(nn & nn2))/(len(nn | nn2))
        e_score_list.append(((node, n) ,jacc))
    s_list = sorted(e_score_list, key=lambda x:(-x[1], x[0][1]))
    s_list.pop(0)
    return s_list[:k]
...
def path_score(graph, root, k, beta):
    ...
    s_scores = sorted(scores, key=lambda x:(-x[1], x[0][1]))
    s_scores.pop(0)
    return s_scores[:k]
